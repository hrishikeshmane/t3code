...
const CLAUDE_CONTEXT_WINDOW_DESCRIPTOR = buildSelectOptionDescriptor({
  id: "contextWindow",
...
const CLAUDE_CONTEXT_WINDOW_CAPABILITIES: ModelCapabilities = createModelCapabilities({
  optionDescriptors: [CLAUDE_CONTEXT_WINDOW_DESCRIPTOR],
});
...
const BUILT_IN_MODELS: ReadonlyArray<ServerProviderModel> = [
  { slug: "auto", name: "Auto", isCustom: false, capabilities: EMPTY_CAPABILITIES },
  {
    ...
    isCustom: false,
    capabilities: CLAUDE_CONTEXT_WINDOW_CAPABILITIES,
  },
  {
  ...
  { slug: "qwen3-coder-next", name: "Qwen 3 Coder Next", isCustom: false, capabilities: EMPTY_CAPABILITIES },
  { slug: "qwen3-coder-480b", name: "Qwen 3 Coder 480B", isCustom: false, capabilities: EMPTY_CAPABILITIES },
  ...
  { slug: "agi-nova-beta-1m", name: "AGI Nova Beta 1M", isCustom: false, capabilities: EMPTY_CAPABILITIES },
];
...
export const checkKiroProviderStatus = Effect.fn("checkKiroProviderStatus")(
  ...
  > {
    ...
    // Discover agents (best-effort, empty array on failure). Surfaced both as
    // a provider-level `agents` list (for the settings UI) and injected into
    // each model's `agentOptions` capability so the composer TraitsPicker
    // renders a per-thread Agent picker.
    const agents = yield* fetchKiroAgents();
    const agentOptions =
      agents.length > 0
        ? agents.map((agent) =>
            agent.isDefault
              ? { value: agent.name, label: agent.name, isDefault: true as const }
    ...
    const modelsWithAgentOptions = agentDescriptor
      ? models.map((model) => ({
          ...model,
