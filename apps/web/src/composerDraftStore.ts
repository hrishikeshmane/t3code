...
function normalizeProviderKind(value: unknown): ProviderKind | null {
  return value === "codex" || value === "claudeAgent" || value === "cursor" || value === "opencode" || value === "kiro"
    ? value
    : null;
}
...
function normalizeProviderModelOptions(
  ...
  const result: Partial<Record<ProviderKind, ReadonlyArray<ProviderOptionSelection>>> = {};
  for (const providerKey of ["codex", "claudeAgent", "cursor", "opencode", "kiro"] as const) {
...
function legacyToModelSelectionByProvider(
  ...
  if (modelOptions) {
    for (const provider of ["codex", "claudeAgent", "cursor", "opencode", "kiro"] as const) {
      const options = modelOptions[provider];
      if (options && options.length > 0) {
